...
dict_wires_second['b'] = dict_wires['a']

while len(dict_wires_second) < len(all_instructions):
    for instruction in all_instructions:
        if len(instruction[0]) == 1:
            if instruction[1] != 'b':
                if is_int(instruction[0][0]) == True:
                    dict_wires_second[instruction[1]] = int(instruction[0][0])
                elif instruction[0][0] in dict_wires_second:
                    dict_wires_second[instruction[1]] = dict_wires_second[instruction[0][0]]
        elif instruction[0][0] == 'NOT':
            if instruction[0][1] in dict_wires_second:
                dict_wires_second[instruction[1]] = bit_not_16(dict_wires_second[instruction[0][1]])
        elif instruction[0][1] == 'AND':
            if is_int(instruction[0][0]) == True:
                if instruction[0][2] in dict_wires_second:
                    dict_wires_second[instruction[1]] = int(instruction[0][0]) & dict_wires_second[instruction[0][2]]
            elif (instruction[0][0] in dict_wires_second) and (instruction[0][2] in dict_wires_second):
                dict_wires_second[instruction[1]] = dict_wires_second[instruction[0][0]] & dict_wires_second[instruction[0][2]]
        elif instruction[0][1] == 'OR':
            if (instruction[0][0] in dict_wires_second) and (instruction[0][2] in dict_wires_second):
                dict_wires_second[instruction[1]] = dict_wires_second[instruction[0][0]] | dict_wires_second[instruction[0][2]]
        elif instruction[0][1] == 'LSHIFT':
            if instruction[0][0] in dict_wires_second:
                dict_wires_second[instruction[1]] = left_shift_16(dict_wires_second[instruction[0][0]], int(instruction[0][2]))
        elif instruction[0][1] == 'RSHIFT':
            if instruction[0][0] in dict_wires_second:
                dict_wires_second[instruction[1]] = dict_wires_second[instruction[0][0]] >> int(instruction[0][2])
                
print(dict_wires_second['a'])
